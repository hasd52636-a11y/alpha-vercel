...
import { KnowledgeItem, AIProvider } from "../types.ts";
...
我们的技术专家会为您提供详细的图片分析和解决方案。`;
  ...
  // 设置智谱API密钥
  public async setZhipuApiKey(key: string): Promise<boolean> {
    ...
    if (typeof window !== 'undefined') {
      try {
        const { setApiKey } = require('../utils/secureStorage');
  ...
  getZhipuApiKey(): string {
    // 优先使用内存中的密钥
    if (this.zhipuApiKey && this.validateApiKey(this.zhipuApiKey)) {
    ...
    try {
      if (typeof import.meta !== 'undefined' && (import.meta as any).env?.VITE_ZHIPU_API_KEY) {
        const envKey = (import.meta as any).env.VITE_ZHIPU_API_KEY;
        if (this.validateApiKey(envKey)) {
          this.zhipuApiKey = envKey;
          return envKey;
        }
      }
    } catch (e) {
    ...
    if (typeof process !== 'undefined' && (process as any).env?.ZHIPU_API_KEY) {
      const envKey = (process as any).env.ZHIPU_API_KEY;
      if (this.validateApiKey(envKey)) {
        this.zhipuApiKey = envKey;
        return envKey;
      }
    ...
    if (typeof window !== 'undefined') {
      try {
        const { getApiKeySync } = require('../utils/secureStorage');
        const secureKey = getApiKeySync();
        if (secureKey && this.validateApiKey(secureKey)) {
          this.zhipuApiKey = secureKey;
          return secureKey;
        }
      } catch (error) {
        console.warn('Secure storage not available, falling back to localStorage');
        ...
        if (typeof localStorage !== 'undefined') {
          ...
          if (savedKey && this.validateApiKey(savedKey)) {
